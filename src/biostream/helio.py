...
def load_auth_key(path=None):
    ...
    if not m:
        raise ValueError("AUTH_KEY (0x + 32 hex) not found in " + path)
    return bytes.fromhex(m.group(1))


class HelioClient:
    def __init__(self, auth_key, strap_name="helio"):
        self.auth_key = auth_key
        self.strap_name = (strap_name or "helio").lower()
    ...
    async def connect_and_auth(self):
        print(f"Scanning for the strap ('{self.strap_name}', 15s)...")
        dev = await BleakScanner.find_device_by_filter(
            lambda d, adv: self.strap_name in (adv.local_name or d.name or "").lower(),
            timeout=15.0,
        )
        if dev is None:
            raise RuntimeError(
                f"strap '{self.strap_name}' not found — ensure it is NOT connected to your phone "
                "(turn off phone Bluetooth). Try `biostream scan` to find the device name.")
        ...
        if pub is None:
            raise RuntimeError("ECDH keypair generation failed")
