"""
Storage layer: SQLite (helio.db) + atomic JSON snapshots (live.json, history.json).
Config-driven — no module-level paths. Pure persistence (no auth, no BLE).
"""
import json
import os
import sqlite3
...
# data-type codes used as sync_state keys (Huami FT_* protocol codes)
FT_ACTIVITY = 0x01
FT_PAI = 0x0d
FT_STRESS_AUTO = 0x13
FT_SPO2_NORMAL = 0x25
...
class Store:
    ...
    def add_hr(self, bpm):
        ...
        del self.hr_trail[:-self.config.history_max]
    ...
    def write_live(self):
        ...
        self._atomic(self.config.live_json, snap)
    ...
    def write_history(self):
        ...
        self._atomic(self.config.history_json, snap)
